 ...
 * Virtual Machine
 *
 * a simple virtual machine. It includes code that
 * 1. executes instructions
 * 2. handles stack frames, sub_stack_frame, operand_stack
 * 3. actually runs the code
 * 4. performs garbage collection. when return.
 *
 * The instructions include jumps, calls, operations like addition and subtraction, and operations related to variables, such as setting and getting global and local variables
 *
 */
...
static vector<StackFrame> callStack; // stack of stack frames
...
auto execute(tuple<vector<Code>, map<string, size_t>> objectCode)->void {
  ...
  while (true) {
    ...
    case Instruction::Return: {
      ...
      collectGarbage(); //return 시 garbage collection 실행
      break;
    }
    ...
    case Instruction::Print: {
      for (size_t i = 0; i < toSize(code.operand); i++) {
        auto value = popOperand(); //this is where actual code runs.
        cout << value;
      }
      break;
    }
...
auto popOperand()->any { //current substack에 operand_stack에 있는 값을 pop해서 반환하는 함수
...
auto collectGarbage()->void {
  for (auto& stackFrame: callStack) {
    for (auto& value: stackFrame.operandStack)
      markObject(value); //mark and sweep 방식
...
auto markObject(any value)->void { //GC method - mark and sweep
  if (isArray(value)) {
